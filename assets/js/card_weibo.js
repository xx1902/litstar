fetch('https://weibo-top-api-one.vercel.app/api').then(data => data.json()).then(data => {
  ...
  // 修改点：使用 slice(0,5) 截取前5条数据
  const top5Data = data.slice(0, 3)

  for (let item of top5Data) {
